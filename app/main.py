...
flow_sites_ref = ['66401']
init_flow_site_ref = '66401'
all_sites_ref = precip_sites_ref + flow_sites_ref
...
def serve_layout():
    ...
    requested_datasets = copy.deepcopy(datasets)

    ### prepare summaries and initial states
    now_date = pd.Timestamp.now(tz='UTC').tz_localize(None).floor('H')
    start_date = now_date - pd.DateOffset(days=2)
    end_date = now_date + pd.DateOffset(days=4)
    ...
    for ds in requested_datasets:
        ...
        sites2 = [s for s in sites1 if s['ref'] in all_sites_ref]
    ...
    flow_site = [s for k, s in sites_dict.items() if s['ref'] == init_flow_site_ref][0]

    flow_lon = flow_site['geometry']['coordinates'][0]
    flow_lat = flow_site['geometry']['coordinates'][1]
    flow_name = flow_site['ref'] + '<br>' + flow_site['name']
    ...
    ## time series data
    flow_forecast_codes = {'high_flow_YR_ECMWF_9km_forecast': 'ECMWF-Yr.no', 'high_flow_wrf_nz8kmN-UKMO': 'UKMO-MetService', 'high_flow_wrf_nz8kmN-ECMWF': 'ECMWF-MetService', 'high_flow_wrf_nz8kmN-NCEP': 'NCEP-MetService'}
    precip_forecast_code = 'YR_ECMWF_9km_forecast'

    meas_flow_ds = [ds for ds in requested_datasets if (ds['parameter'] == 'streamflow') and (ds['processing_code'] == 'raw_data')][0]
    meas_flow_ds_id = meas_flow_ds['dataset_id']

    meas_precip_ds = [ds for ds in requested_datasets if (ds['parameter'] == 'precipitation') and (ds['processing_code'] == 'raw_data')][0]
    meas_precip_ds_id = meas_precip_ds['dataset_id']

    precip_site_ids = [k for k, s in sites_dict.items() if s['ref'] in precip_sites_ref]

    for_precip_ds = [ds for ds in requested_datasets if (ds['parameter'] == 'precipitation') and (ds['processing_code'] == precip_forecast_code)][0]
    ...
    # flow
    meas_ts_r = requests.get(base_url + 'time_series_results', params={'dataset_id': meas_flow_ds_id, 'site_id': flow_site['site_id'], 'compression': 'zstd', 'from_date': start_date, 'to_date': end_date})
    dc = zstd.ZstdDecompressor()
    meas_ts1 = pd.DataFrame(orjson.loads(dc.decompress(meas_ts_r.content)))
    meas_ts1['from_date'] = pd.to_datetime(meas_ts1['from_date'])
    ...
    for code in flow_forecast_codes:
        for_flow_ds = [ds for ds in requested_datasets if (ds['parameter'] == 'streamflow') and (ds['processing_code'] == code)][0]
        for_flow_ds_id = for_flow_ds['dataset_id']

        for_ts_r = requests.get(base_url + 'time_series_results', params={'dataset_id': for_flow_ds_id, 'site_id': flow_site['site_id'], 'compression': 'zstd', 'from_date': start_date, 'to_date': end_date})
        dc = zstd.ZstdDecompressor()
        for_ts1 = pd.DataFrame(orjson.loads(dc.decompress(for_ts_r.content)))
        for_ts1['from_date'] = pd.to_datetime(for_ts1['from_date'])
        for_ts1 = for_ts1.loc[for_ts1['from_date'] > meas_ts1_max]

        x1 = for_ts1.from_date + pd.DateOffset(hours=12)
        y1 = for_ts1.result
    ...
    layout = dict(title = 'Forecasted Flow', paper_bgcolor = '#F4F4F8', plot_bgcolor = '#F4F4F8', xaxis = dict(title="Date (NZST)", range = [start_date, end_date]), showlegend=True, height=ts_plot_height, yaxis=dict(title="Flow (m^3/s)", titlefont=dict(color="#1f77b4"), tickfont=dict(color="#1f77b4")), yaxis2=dict(title="Precip (mm)", titlefont=dict(color="#ff7f0e"), tickfont=dict(color="#ff7f0e"), anchor="x", overlaying="y", side="right"))
...
#     className='two columns', style={'margin': 20}),
    ...
    html.Div([
        html.P('Sites:', style={'display': 'inline-block'}),
        dcc.Graph(
                id = 'site-map',
                style={'height': map_height},
                figure=dict(
                    data = [dict(lat = precip_lat,
                            lon = precip_lon,
                            text = precip_names,
                            type = 'scattermapbox',
                            hoverinfo = 'text',
